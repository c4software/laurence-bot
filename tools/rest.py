...
def callrest(domain="", port="", path="/", type="GET", params=None, timeout=60, encode_post_param_as_json=False,
             ssl=False, user_headers={}, loop=1):
    if params is None:
        params = {}
    connection = False
    ...
    try:
        ...
        if type == "POST":
            if encode_post_param_as_json:
                ...
                headers = override_headers(
                    {"User-Agent": USER_AGENT, "Content-type": "application/x-www-form-urlencoded", "Accept": "*/*"},
                    user_headers)
        ...
        if type == "GET":
            ...
            connection.request('GET', "{0}?{1}".format(path, params), headers=headers)
        ...
        if result.status == 302 or result.status == 301:
            if result.getheader("location", False) or result.getheader("Location", False):
                ...
                print(o)
                return callrest(domain=o.netloc, path=urllib.parse.quote(o.path.encode("utf-8")),
                                user_headers=user_headers, params=params, type=type, loop=loop + 1)
